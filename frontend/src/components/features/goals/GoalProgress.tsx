...
export function GoalProgress({
  current, target, metricType, unit, showLabel = true, size = 'md', className,
}: GoalProgressProps) {
  ...
  return (
    <div className={cn('w-full', className)}>
      {showLabel && (
        <div className="mb-1.5 flex items-center justify-between text-xs">
          <span className="font-mono font-semibold text-slate-700">
            ...
            <span className="font-normal text-slate-400">/ {formatValue(target, metricType, unit)}</span>
          </span>
          <span className={cn(
      ...
      <div className={cn('w-full overflow-hidden rounded-full bg-slate-100', barHeight)}>
        <div className={cn(
          'rounded-full transition-all duration-500', barHeight,
...
function formatValue(value: number, metricType?: MetricType, unit?: string): string {
  ...
  if (unit === '小时') return `${value.toFixed(1)}h`;
  return `${value}${unit || ''}`;
}
