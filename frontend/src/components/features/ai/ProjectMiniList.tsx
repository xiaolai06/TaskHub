  ...
  if (projects.length === 0) {
    return (
      <div className="py-6 text-center">
        <p className="text-[11px] text-slate-400">暂无活跃项目</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-0.5">
      ...
      {projects.slice(0, 5).map((p) => (
        <div
          key={p.id}
          className="flex items-center justify-between rounded-lg px-2 py-2 transition-colors hover:bg-white hover:shadow-sm"
        >
          <div className="min-w-0 flex-1">
            <p className="truncate text-[12px] font-medium text-slate-700">{p.name}</p>
          ...
          <span className={cn(
            ...
            statusCN[p.status] || 'bg-slate-50 text-slate-500',
          )}>
