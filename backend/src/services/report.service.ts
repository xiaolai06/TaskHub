...
export async function getOverview(userId: string, period?: string, type?: string) {
  ...
  const paidOrders = await prisma.project.findMany({
    where: { ownerId: userId, status: 'COMPLETED', updatedAt: { gte: start, lte: end } },
    select: { id: true, budget: true },
  });
  const monthlyIncome = paidOrders.reduce((sum, project) => sum + (project.budget ?? 0), 0);
...
export async function getProjectRanking(userId: string, period?: string, type?: string) {
  ...
  const projectIds = projects.map((project) => project.id);
  const [recordCostMap, taskCostMap] = await Promise.all([
    getRecordCostByProject(projectIds),
    getTaskCostByProject(projectIds),
  ]);
  ...
  const ranking = projects.map((project) => {
    ...
    const margin = quote > 0 ? Math.round((profit / quote) * 1000) / 10 : 0;
    const paidInPeriod = project.status === 'COMPLETED' && project.updatedAt >= start && project.updatedAt <= end;
    return {
      id: project.id,
      name: project.name,
      budget: quote,
