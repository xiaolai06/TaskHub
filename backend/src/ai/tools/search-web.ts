...
async function getSearchConfig(userId: string): Promise<SearchConfig> {
  ...
  if (rawKey) {
    try { apiKey = decrypt(rawKey); } catch { logger.warn('Search API Key 解密失败，请重新配置'); apiKey = ''; }
  }

  let parsedCfg: SearchConfigRaw = {};
...
async function callDuckDuckGo(query: string, cfg: SearchConfig, proxyUrl?: string) {
  ...
  if (!proxyUrl) {
    ...
    try {
      ...
      logger.warn('Search duck-duck-scrape 直连失败，切换到 Lite 版本');
    }
  }
...
AI 自适应提示: 质量最高的搜索引擎，但需要付费 Key。未配置 Key 时自动不可用，国内网络用 search_sogou 替代，有代理用 search_duckduckgo 替代。`,
  ...
  handler: async (args, userId) => {
    ...
    if (cached) {
      logger.debug({ query: query.slice(0, 30), cacheKey }, 'Search 缓存命中');
      return cached;
    }
    ...
    try {
      ...
      logger.warn({ provider, err }, 'Search provider failed');
...
AI 自适应提示: 免费搜索，但需要代理（国内被墙）。无代理时请用 search_sogou 替代。配置代理后会自动通过代理访问。`,
  ...
  handler: async (args, userId) => {
    ...
    if (cached) {
      logger.debug({ query: query.slice(0, 30) }, 'Search 缓存命中 (ddg)');
      return cached;
    }
    ...
    try {
      ...
      logger.warn({ err }, 'Search duckduckgo failed');
